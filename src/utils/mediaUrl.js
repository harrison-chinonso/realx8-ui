...
export function resolveMedia(url, storedType) {
  ...
  if (!raw) return { kind: 'image', src: '', provider: null, poster: null };
  ...
  if (VIDEO_EXTS.test(direct)) return { kind: 'video', src: direct, provider: null, poster: null };
  if (IMAGE_EXTS.test(direct)) return { kind: 'image', src: direct, provider: null, poster: null };

  // No extension to go on — fall back to whatever was recorded at save time.
  if (storedType === 'video') return { kind: 'video', src: direct, provider: null, poster: null };
  return { kind: 'image', src: direct, provider: null, poster: null };
}
